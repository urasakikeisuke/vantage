 ...
 * APIルートからの内部呼び出し用
 */
export async function fetchStockPricesServer(
  symbols: string[],
  options: FetchStockPricesOptions = {},
): Promise<StockPrice[]> {
  ...
  const quotes = await Promise.all(
    symbols.map(async (symbol) => {
      ...
      try {
        ...
        if (!quote) {
          console.error(`Error fetching ${symbol}: quote is undefined`);
          return null;
        }

        let sector = "その他";
        let quoteType = quote.quoteType || "EQUITY";
